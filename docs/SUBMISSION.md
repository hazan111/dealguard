...
3. **Real per-agent identity with an IAM-enforced zero-trust boundary.** The commercial tools run on their own closed SaaS stacks. Each DealGuard agent runs on Agent Engine under its **own dedicated Google Cloud service account** — five separate, auditable identities. The zero-trust boundary is enforced at the IAM level, not just in application code: only the Orchestrator's identity holds access to the risk-register Firestore database (which stores the documents, findings, and audit trail); the four specialists' identities hold a *conditional* grant scoped exclusively to the A2A task-store database. We verified this empirically, not just by reading the policy: authenticating as the Legal agent's service account and attempting to read the risk register returns `403 PERMISSION_DENIED` from GCP itself, while the same identity's read of the A2A task store succeeds. To be precise about the one layer that is application-level: which document *content* a specialist receives (Legal agent only ever gets legal-classified text) is decided by the gateway's routing, since content flows to specialists over A2A rather than being pulled from storage they could browse. The platform's Agent Identity API surface (authProviders) is enabled on the project; adopting it beyond IAM service accounts is listed under Known Limitations.
...
One security finding surprised us enough to change the gateway mid-build. When we first ran the poisoned seed document through Model Armor, the whole-document scan came back clean: a few sentences of injected instructions surrounded by a page of plausible financial reporting diluted the classifier signal below the detection threshold, while the same injection paragraph scanned in isolation was flagged immediately. Scanning the document once, end to end, was simply not enough. The gateway now screens every document twice — the full text, plus overlapping ~800-character segments — and a match in any single segment blocks the document. (We also had to lower the filter's confidence threshold from MEDIUM to LOW_AND_ABOVE; at MEDIUM even the isolated injection text passed.) The dilution effect is easy to miss if you only ever test guardrails against bare attack strings rather than attacks embedded in realistic carrier documents, which is exactly what a malicious data-room submission would look like.
...
- Drive API watch channels expire quickly (~1 hour observed on this folder) and renewal is currently **manual**: the demo script re-arms the channel each time it is run (plus re-registration on container startup) — there is no automated scheduler, so real-time push silently lapses between sessions until the script is re-run. A production build would need scheduled renewal.
- The system is a decision-support tool, not a decision-maker: every finding requires human review and sign-off before being marked resolved. The name on that sign-off is taken from the authenticated session server-side, never from the request body, so a client can ask to resolve a finding but cannot choose whose name goes on the record.
