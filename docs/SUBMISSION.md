...
3. **Real cryptographic agent-level identity.** The commercial tools run on their own closed SaaS stacks. DealGuard's agents each hold a real Agent Identity (SPIFFE-based) — a genuine, cryptographically verifiable identity for each agent, not something a single-vendor legal-tech product built on its own infrastructure would have any structural reason to replicate. To be precise about scope: in this hackathon build, the *identity* itself is real and platform-native; the zero-trust *access boundary* (Legal agent cannot read HR documents, etc.) is enforced at the application/routing layer, not yet via full IAM policy — see Known Limitations below for exactly where that line sits.
...
- Zero-trust document access is enforced at the application/routing layer in this build; full IAM-policy-level enforcement via Agent Identity is a natural next step beyond hackathon scope.
